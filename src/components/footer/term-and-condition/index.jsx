...
const TermAndCondition = () => {
    ...
    const openModal = () => {
        setNewPrivacy({ termAC_title: allData[0] && allData[0].termAC_title, termAC_description: privacyData })
        setIsModalOpen(true);
    }
    ...
    const fetchBrandData = async () => {
        ...
        try {
            ...
            setPrivacyData(response.data[0].termAC_description);
    ...
    const handleAddPrivacy = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await fetchApi({ url: `${BASE_URL}/term`, isAuthRequired: true, method: "PUT", data: newPrivacy });
        ...
        <>
            <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-lg font-bold">
                        ...
                        ) : (allData[0] && allData[0].termAC_title)}
                    </h2>
                    <button
                    ...
                    <table className="min-w-full bg-white shadow-md rounded-lg">
                        <thead className="bg-gray-800 text-white">
                            <tr>
                                ...
                                <th className="py-3 px-5 border border-gray-400 text-left text-sm font-semibold">Term Title</th>
                                <th className="py-3 px-5 border border-gray-400 text-left text-sm font-semibold">Term Description</th>
                        ...
                        <tbody>
                            {isloadingMainData ? (
                                ...
                                <tr>
                                    ...
                                    >
                                        No Terms and condition available
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            >
                <div className="bg-white rounded-lg">
                    <h2 className="text-2xl font-bold text-gray-800 mb-4">Change Terms</h2>
                    <form onSubmit={handleAddPrivacy}>
                        <input
                            ...
                            value={newPrivacy.termAC_title}
                            onChange={(e) => setNewPrivacy({ ...newPrivacy, termAC_title: e.target.value })}
                        />
                        <button
                            type='button'
                            onClick={() => {
                                // console.log(newPrivacy.termAC_description[-1])
                                // if(newPrivacy.termAC_description[-1] && newPrivacy.termAC_description[-1].title !== "" && newPrivacy.termAC_description[-1].desc !== ""){
                                setNewPrivacy({ ...newPrivacy, termAC_description: [...newPrivacy.termAC_description, { title: "", desc: "" }] })
                                // }
                            }}
                            className='bg-blue-600 hover:bg-blue-800 text-white py-1 px-3 rounded-lg text-md mb-3'>Add Privacy</button>
                        {newPrivacy.termAC_description && newPrivacy.termAC_description.map((e, i) => (
                            <div key={i}>
                                <div className='flex justify-between'>
                                    <p className='text-blue-700 my-2'>Terms - {i + 1}</p>
                                    <button
                                        onClick={() => setNewPrivacy({ ...newPrivacy, termAC_description: newPrivacy.termAC_description.filter((ele, index) => index !== i && ele) })}
                                ...
                                <input
                                    ...
                                    placeholder="Title"
                                    className="border border-gray-300 p-3 rounded-lg mb-3 w-full focus:outline-none focus:ring-2 focus:ring-blue-400"
                                    value={newPrivacy.termAC_description[i].title}
                                    onChange={(e) => setNewPrivacy({ ...newPrivacy, termAC_description: newPrivacy.termAC_description.map((ele, index) => index === i ? { ...ele, title: e.target.value } : ele) })}
                                />
                                <textarea
                                    placeholder="Description"
                                    className="border border-gray-300 p-3 rounded-lg mb-3 w-full focus:outline-none focus:ring-2 focus:ring-blue-400"
                                    value={newPrivacy.termAC_description[i].desc}
                                    onChange={(e) => setNewPrivacy({ ...newPrivacy, termAC_description: newPrivacy.termAC_description.map((ele, index) => index === i ? { ...ele, desc: e.target.value } : ele) })}
                                />
                                {/* <ReactQuill
                                    theme="snow"
                                    onChange={(value) => {
            ...
            >
                ...
                <div className="bg-white p-2">
                    <h2 className="text-2xl font-bold mb-4 text-center">Term Details</h2>

                    <div className="mb-4">
                        <h3 className="text-lg font-semibold mb-1">Term Title</h3>
                        <p className="text-gray-700">{viewData.title}</p>
                    </div>

                    <div className="mb-4">
                        <h3 className="text-lg font-semibold mb-1">Term Content</h3>
